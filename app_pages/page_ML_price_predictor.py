...
def page_ML_price_predictor_content():
    ...
    st.info(
        ...
        "Prior to creating the pipeline, we conducted a "
        "comprehensive feature engineering analysis. This process involved "
        "transforming raw data into meaningful features to enhance the "
        "predictive power of our models. During the feature engineering "
        "phase, "
        "we focused on several key steps. We started by checking for missing "
        "values and validating analysis types. Next, we performed normality "
        "tests using the Shapiro-Wilk method to identify which variables "
        "required transformation. We then applied various transformations, "
        "such as logarithmic, power, Box-Cox, Yeo-Johnson, ordinal encoding, "
        "and Winsorization, to address data skewness and normalize "
        "distributions. "
        "The effectiveness of these transformations was evaluated using the "
        "Shapiro-Wilk test to ensure improved normality. Additionally, we "
        "implemented feature selection techniques to handle multicollinearity "
        "and select the most relevant features. These steps aimed to enhance "
        "data quality and distribution, ultimately improving model "
        "performance.")

    st.write("### Create Model Pipeline")
    st.info(
        "**Implement Pipeline**\n\n"
        "* Create a model base to handle missing values and encoding"
        "strategies.\n"
    ...
    st.info(
        ...
        " showing a clear, positive relationship between OverallQual and "
        "SalePrice.\n"
        " We believe the correlation level is accurate\n"
        "* The permutation importance method further validates the model's "
        "balanced reliance on multiple features, ensuring robustness and "
        "interpretability. These findings suggest that OverallQual is a "
        "crucial predictor of house prices in the dataset, but the model also "
        "benefits from incorporating other significant features.\n"
        "* Through hyperparameter optimization of the "
        " GradientBoostingRegressor, the"
        " best configuration achieved a mean score of 0.773, which is "
        "comparable to the initial ExtraTreeRegressor model.\n"
